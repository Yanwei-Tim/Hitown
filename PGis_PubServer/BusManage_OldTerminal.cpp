...
void CBusinessManage_OldTerminal::OnSendErrorSvr(const DWORD dwClientContext, const char* pBuf, const DWORD nBytes, const DWORD dwInstaned)
{
	...
	DWORD dwPubContext;
	temp_this->m_HashmapDataInfo.FindItem(dwClientContext,dwPubContext);
...
{ 
	...
	{	
		...
		{   
			...
			gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d]相同DeviceCode终端登录，强制断开之前的连接 DeviceCode:%s\n 总登陆人数：%d\n",CTime::GetCurrentTime().Format("%H:%M:%S"),
