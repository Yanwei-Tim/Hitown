...
{
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] [PUB->NEWTER] OnReceiveErrorSvr \n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext);	
}
...
{
	gWriteLogApp.WriteLog(FOR_TRACE,"%s [SYS] [ID:%d] [PUB->NEWTER] SendData:%s \n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext,pBuf);
}
...
{
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] [PUB->NEWTER] ERROR SendData:%s \n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext,pBuf);

	CBusinessManage_NewTerminal* temp_this = (CBusinessManage_NewTerminal*)dwInstaned;
	char tempSendBuf[512] = {0};
	DATA_INFO tempdatainfo;
	ZeroMemory(&tempdatainfo,sizeof(DATA_INFO));
	temp_this->m_HashmapDataInfo.FindItem(dwClientContext,tempdatainfo);
	sprintf(tempSendBuf,"%s,%s,0",tempdatainfo.strDeviceCode,tempdatainfo.strDeviceType);
	...
	temp_this->m_HashmapDevid.Delete(tempdatainfo.strDeviceCode);
	temp_this->m_HashmapDataInfo.Delete(dwClientContext);
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] %s %s 发送数据错误 退出 [新终端在线总数:%d]\n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext,
		tempdatainfo.strDeviceCode,tempdatainfo.strDeviceType,temp_this->m_HashmapDataInfo.GetSize());
}
...
void CBusinessManage_NewTerminal::OnIdleOvertime(const DWORD dwClientContext, const DWORD dwInstaned)
{
	...
	temp_this->m_HashmapDevid.Delete(tempdatainfo.strDeviceCode);
	temp_this->m_HashmapDataInfo.Delete(dwClientContext);
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] %s %s 心跳超时 退出 [新终端在线总数:%d]\n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext,
		tempdatainfo.strDeviceCode,tempdatainfo.strDeviceType,temp_this->m_HashmapDataInfo.GetSize());
}
...
void CBusinessManage_NewTerminal::OnConnectionOverflow(const DWORD dwClientContext, const DWORD dwInstaned)
{
	...
	temp_this->m_HashmapDevid.Delete(tempdatainfo.strDeviceCode);
	temp_this->m_HashmapDataInfo.Delete(dwClientContext);
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] %s %s 连接数超过上限 退出 [新终端在线总数:%d]\n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext,
		tempdatainfo.strDeviceCode,tempdatainfo.strDeviceType,temp_this->m_HashmapDataInfo.GetSize());
}
...
// 			gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] %s %s 客户端主动 退出 [新终端在线总数:%d]\n",CTime::GetCurrentTime().Format("%H:%M:%S"),dwClientContext,
// 				tempdatainfo.strDeviceCode,tempdatainfo.strDeviceType,m_HashmapDataInfo.GetSize());
...
{ 
	...
	{	
		...
		{   
			...
			gWriteLogApp.WriteLog(FOR_ALARM,"%s [SYS] [ID:%d] 相同DeviceCode终端登录，强制断开之前连接 DeviceCode:%s\n",CTime::GetCurrentTime().Format("%H:%M:%S"),
