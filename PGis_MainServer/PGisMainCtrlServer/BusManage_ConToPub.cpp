...
BOOL CBusinessManage_ConToPub::ProessAnalyseData(const DWORD dwClientContext, const char* RecvData, const DWORD dwRecvLen, int nCmdType)
{
	...
	case A_TYPE_TERMINAL_GPS: //终端上传GPS协议
		...
		gPGisMainCtrlProc.m_DBManage_WebServer.ReportWebServiceRecvGps(dwClientContext,RecvData,dwRecvLen);				//GPS数据 写数据库
...
void CBusinessManage_ConToPub::SendData_Broadcast_GisClient_TerminalGpsData(const char *RecvData, int nReturnIndex)
{
	...
	gPGisMainCtrlProc.m_BusinessManage_GISTerminal.SendData_Broadcast(sendBuf,strlen(sendBuf),C_TYPE_TERMINAL_GPS,0,CBusinessManage_GISTerminal::GisTer_All);


// #ifdef WANGZH_LOCATION //2013-5-2
// 	//如果地理信息服务器没有连接
// 	char sendLocatBuf[MAX_DATA_PACKAGE_LEN] = {0};
// 	int nSendLocatInfoLen = 0;
// 	if(!m_TcpLocationServer
// 		.GetStartFlag() && m_bConnLocationServer)
// 	{
// 		sendLocatBuf[0] = PACKAGE_HEAD_DATA_STX;
// 		nSendLocatInfoLen = 1 + strlen(pData[0]) + strQueryIndex.GetLength() + 10;
// 		_snprintf(sendLocatBuf+1,MAX_DATA_PACKAGE_LEN - 2,
// 			"%.4X0D%s,%d, ",
// 			nSendLocatInfoLen,
// 			pData[0],
// 			pTempGpsData->returnIndex);
// 		sendLocatBuf[nSendLocatInfoLen - 1] = PACKAGE_TAIL_DATA_ETX;
// 	}
// #endif
// 
// 	BOOL bFlag = FALSE;
// 	for(i = 0 ; i < MAX_GIS_CLIENT_COUNT; i++)
// 	{
// 		if(m_TableGisClient[i].CheckSendState(strDeptNum))
// 		{
// 			if(m_TcpServGisSocket.Send(i,sendBuf,sendLen) == TCPSERVER_ERROR_SEND)
// 			{
// 				m_TcpServGisSocket.CloseClient(i);
// 			}
// 			else
// 			{
// 				bFlag = TRUE;
...
// 
// #ifdef WANGZH_LOCATION //2013-5-2
// 			if(!m_TcpLocationServer.GetStartFlag() && m_bConnLocationServer)
// 			{
// 				//发送一条空的地理位置信息
// 				if(m_TcpServGisSocket.Send(i,sendLocatBuf,nSendLocatInfoLen) == TCPSERVER_ERROR_SEND)
// 				{
// 					m_TcpServGisSocket.CloseClient(i);
// 				}
// 			}
...
// 	if(bFlag)
// 	{
// 		sendBuf[sendLen - 1] = NULL;
// 		gWriteLogApp.WriteLog(FOR_DEBUG,_T("%s [GIS]向设备所属部门GIS客户端广播GPS数据:data:%s\n"),
// 			CTime::GetCurrentTime().Format("%H:%M:%S"),sendBuf+1);
// 	}
}
