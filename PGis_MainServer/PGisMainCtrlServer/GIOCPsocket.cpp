...
BOOL GIOCPsocket::Start()
{
	...
	if (OnStartIOCPSocket(m_dwMaxConnect,m_dwAcceptTime,m_dwIdleOverTime,m_dwHeartBeatTime,m_dwTimeAutoConnect,m_dwIODataCount,m_dwGBufSize,m_nSocketBuffer))
	{
		m_IsIOCPActive = TRUE;
...
BOOL GIOCPsocket::OnStartIOCPSocket(DWORD dwMaxConnection_SVR,		//最大连接量，默认仅需修改最大连接量即可
									...
									DWORD dwIoDataTotal_SOCKET,		//每秒单终端发送数据次数
									...
									int	  nSocketBufSize_SOCKET)
{
	...
	m_dwIoDataTotal = ((dwIoDataTotal_SOCKET * dwMaxConnection_SVR * 1.1) > DWORD(-1)) ? DWORD(-1):(dwIoDataTotal_SOCKET * dwMaxConnection_SVR * 1.1);
...
void GIOCPsocket::OnIdleOvertime(const DWORD dwClientContext)
{
	GPFNDATA_SVR temp_gpfndata_svr;
	ZeroMemory(&temp_gpfndata_svr,sizeof(GPFNDATA_SVR));
	((GIOCPsocket*)GHndDat_GetOwner((DWORD)GHndDat_GetOwner(dwClientContext)))->m_GPfnData_SVR_HashManager.FindItem((DWORD)GHndDat_GetOwner(dwClientContext),temp_gpfndata_svr);
...
void GIOCPsocket::OnConnectionOverflow(const DWORD dwClientContext)
{
	GPFNDATA_SVR temp_gpfndata_svr;
	ZeroMemory(&temp_gpfndata_svr,sizeof(GPFNDATA_SVR));
	((GIOCPsocket*)GHndDat_GetOwner((DWORD)GHndDat_GetOwner(dwClientContext)))->m_GPfnData_SVR_HashManager.FindItem((DWORD)GHndDat_GetOwner(dwClientContext),temp_gpfndata_svr);
...
void GIOCPsocket::OnHeartbeat(const DWORD dwClientContext)
{
	GPFNDATA_CLT temp_gpfndata_clt;
...
void GIOCPsocket::OnConnectError(const DWORD dwClientContext)
{
	GPFNDATA_CLT temp_gpfndata_clt;
	ZeroMemory(&temp_gpfndata_clt,sizeof(GPFNDATA_CLT));
	((GIOCPsocket*)GHndDat_GetOwner(dwClientContext))->m_GPfnData_CLT_HashManager.FindItem(dwClientContext,temp_gpfndata_clt);
