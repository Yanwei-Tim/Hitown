...
void CBusinessManage_ConnectToShanhaiServer::OnReceiveClt(const DWORD dwClientContext, const char* pRecvData, const DWORD nRecvLen, const DWORD dwInstaned)
{
	...
	if(temprecvbufferinfo.nBufLenCount + nRecvLen > MAX_RECVICE_BUFFER)
	{
		...
		TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d] ERROR 处理数据包超长为 %s %d \n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,temprecvbufferinfo.RecvBuf,temprecvbufferinfo.nBufLenCount);		
		return /*FALSE*/;
	}
	...
	TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d] 需要处理的数据包长为 %s %d \n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,temprecvbufferinfo.RecvBuf,temprecvbufferinfo.nBufLenCount);
	...
	while(nSpaceLen >= MAIN_DATA_PACKAGE_LEN)
	{
		...
		if (-1 == nHeadPoint)
		{
			TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d] 包头错:len:%d,recv:%s\n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,nSpaceLen,temppHeadCurPoint);
			gWriteLogApp.WriteLog(FOR_ALARM,"%s [Shanhai_%d] 整包%s 截取后的包头错 %s,没有找到%2X\n",CTime::GetCurrentTime().Format("%H:%M:%S"),temp_this->m_nIndexServer,pRecvData,temppHeadCurPoint,us_LocationHead);
			nSpaceLen = 0;
			break;
		}
		...
		if( nTailPoint > 0 )
		{
			nPackageLen = nTailPoint;
			if( nPackageLen < MAX_DATA_PACKAGE_LEN )
			{
				temp_this->ProessAnalyseData(dwClientContext,pCurPoint,nPackageLen);
				TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d] 完整包长为%s %d \n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,pCurPoint+2,nPackageLen-2);
			}
			else
			{
				TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d]包长度超过允许的最大值:len:%d,recv:%s\n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,nSpaceLen,pRecvData);
			}
		...
		else
		{
			...
			{
				TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d]没有找到包尾:len:%d,recv:%s\n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,nSpaceLen,pCurPoint);
				gWriteLogApp.WriteLog(FOR_ALARM,"%s [Shanhai_%d] 整包%s 截取后的包尾错 %s,没有找到%4x\n",CTime::GetCurrentTime().Format("%H:%M:%S"),temp_this->m_nIndexServer,pRecvData,ptempTailCurPoint,us_LocationHead);
			}
			nSpaceLen = 0;
			break;
		}
	...
	if( nSpaceLen > 0 )
	{
		...
		{
			TRACE(_T("[Shanhai_%d][THREAD:%d][ID:%d] 没有处理的数据包长为 %s %d \n"),temp_this->m_nIndexServer,GetCurrentThreadId(),dwClientContext,temprecvbufferinfo.RecvBuf,temprecvbufferinfo.nBufLenCount);
...
{
	CBusinessManage_ConnectToShanhaiServer* temp_this = (CBusinessManage_ConnectToShanhaiServer*)dwInstaned;
	gWriteLogApp.WriteLog(FOR_TRACE,"%s [Shanhai_%d][ID:%d] SendData:%s \n",CTime::GetCurrentTime().Format("%H:%M:%S"),temp_this->m_nIndexServer,dwClientContext,pBuf);
}
...
{
	CBusinessManage_ConnectToShanhaiServer* temp_this = (CBusinessManage_ConnectToShanhaiServer*)dwInstaned;
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [Shanhai_%d][ID:%d] SendErrorData:%s 发送错误退出 \n",CTime::GetCurrentTime().Format("%H:%M:%S"),temp_this->m_nIndexServer,dwClientContext,pBuf);
}
...
void CBusinessManage_ConnectToShanhaiServer::OnConnectError(const DWORD dwClientContext, const DWORD dwInstaned)
{
	...
	gWriteLogApp.WriteLog(FOR_ALARM,"%s [Shanhai_%d][ID:%d] OnConnectError 继续重连\n",CTime::GetCurrentTime().Format("%H:%M:%S"),temp_this->m_nIndexServer,dwClientContext);
}
...
BOOL CBusinessManage_ConnectToShanhaiServer::ProessAnalyseData(const DWORD dwClientContext, const char* RecvData, const DWORD nRecvLen)
{
	...
	SH_Head* ptemp_head = (SH_Head*)RecvData;

// 	unsigned short us_Version = 0;
// 	unsigned short us_CmdType = 0;
//	sscanf(tempBuf, "%2x%2x%4x",&us_CmdType,&us_Version,&nTempLen);	
//	sscanf(tempBuf, "%2x",&us_CmdType);
...
// 	}
	...
	ProessAnalyseData(dwClientContext,/*tempBuf+6,nTempLen,*/ptemp_head->us_cmd);
...
// 	}
	...
	case SH_TYPE_DISCONNET: //断开
		...
		gWriteLogApp.WriteLog(FOR_ALARM,"%s [Shanhai_%d][ID:%d] 山海经通知退出 \n",CTime::GetCurrentTime().Format("%H:%M:%S"),m_nIndexServer,dwClientContext);
		break;
	default:
		break;
	}
...
BOOL CBusinessManage_ConnectToShanhaiServer::EventLogin(const DWORD dwClientContext)
{
	...
	pLoginData->Head.un_Len = sizeof(SH_LoginData);
...
BOOL CBusinessManage_ConnectToShanhaiServer::EventHeartBeat(const DWORD dwClientContext)
{
	...
	pLoginData = (SH_LoginData *)sendBuf;
	pLoginData->Head.us_Header = SH_HEAD;
	pLoginData->Head.us_cmd= SH_TYPE_TICK;
	pLoginData->Head.us_version = SH_VERION;
	pLoginData->Head.un_Len = sizeof(SH_LoginData);
	strncpy(pLoginData->UserId,m_strSHLoginID,sizeof(m_strSHLoginID));
	strncpy(pLoginData->UserPwd,m_strSHLoginPSWD,sizeof(m_strSHLoginPSWD));
...
{	
	...
	pGpsData->Head.un_Len = sizeof(SH_GpsData);
