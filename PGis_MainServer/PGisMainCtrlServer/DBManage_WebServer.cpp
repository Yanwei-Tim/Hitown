...
// 	int nRes = GetTerminInfo(strDeviceCode,nTempUserType,strTempUserCode,strDeptNum,bPatrolStart);//-1 是没有用户
// 	if(nRes == -1)
// 	{
// 		AddDeviceLoginTerminalList(pData[0],"TC890GM"); //直接将该终端加到再线列表中
// 		//终端号码没有在系统登记 重新登记之+++++++++++++++++++++++++++
// 		if(m_dwWorkThreadID > 0  && g_nRecvConntGisClientCount > 0)
// 		{
// 			int nTempDevNumLen = 0;
// 			nTempDevNumLen = strlen(pData[0]);
// 			char *pTempDeviceNum = new char[nTempDevNumLen+1];
// 			memset(pTempDeviceNum,0,nTempDevNumLen+1);
// 			memcpy(pTempDeviceNum,pData[0],nTempDevNumLen);
// 
// 			//通知线程模拟一条终端上线信息
// 			PostThreadMessage(m_dwWorkThreadID,WM_TERMINAL_ONLINE_MESSAGE,(WPARAM)pTempDeviceNum,0);
// 		}
// 
// 		if(WebService_QueryDB_PubTerminalInfo(strDeviceCode,nTempUserType,strTempUserCode,strDeptNum))
// 		{
// 			gWriteLogApp.WriteLog(FOR_TRACE_1,_T("%s WEBService查询设备信息,DeviceID:%s,UserType:%d,UserCode:%s,DeptNum:%s\n"),
// 				CTime::GetCurrentTime().Format("%H:%M:%S"),
// 				strDeviceCode,
// 				nTempUserType,
// 				strTempUserCode,
// 				strDeptNum);
// 
// 			WriteTerminalUserInfoToList(strDeviceCode,nTempUserType,strTempUserCode,strDeptNum);
// 		}
// 
// 		return TRUE; 
// 	}
// 	else if(nRes == 0) //设备已经在系统登录但是没有查询到设备对应的信息
// 	{
// 		if(WebService_QueryDB_PubTerminalInfo(strDeviceCode,nTempUserType,strTempUserCode,strDeptNum))
// 		{
// 			gWriteLogApp.WriteLog(FOR_TRACE_1,_T("%s WEBService查询设备信息,DeviceID:%s,UserType:%d,UserCode:%s,DeptNum:%s\n"),
...
// 		else
// 		{
// 			//TRACE("%s 收到GPS数据但数据库没有对应的设备信息_1,Device:%s\n",CTime::GetCurrentTime().Format("%H:%M:%S"),strDeviceCode);
// 			DisPatchGpsDataToWorkThread(pGpsData,-1,"");
// 			return TRUE; //在数据库中没有查询到设备信息说明,不用写库直接返回成功
// 		}
// 	}

	//nRes 结果= 1,则往下执行 说明设备在系统登记，并在数据库中有记录
	...
	if(gPGisMainCtrlProc.m_BusinessManage_ConnectToGPSCheckServer.m_bConnGpsCheckServer && /*bPatrolStart && */nGpsDataType != GPS_DATA_HISTORY)
	{
	...
	{
		if (gPGisMainCtrlProc.m_BusinessManage_ConnectToLocationServer.QueryLocationProcess(/*nReturnIndex*/dwClientContext,pData[0],pData[6],pData[4]))	//返回信息
		{
	...
	for (size_t nCount = 0;nCount< gPGisMainCtrlProc.m_BusinessManage_ConnectToShanhaiServer_vec.size();++nCount)
	{
		...
		if(gPGisMainCtrlProc.m_BusinessManage_ConnectToShanhaiServer_vec[nCount]->m_bConnShanhaiServer && gPGisMainCtrlProc.m_SubscriptionManager.GetSubscriptionInfo_ShanHai(gPGisMainCtrlProc.m_BusinessManage_ConnectToShanhaiServer_vec[nCount]->m_szSubscriptionID,(DWORD)&temp_TermainalInfo))
		{
			...
			CString strTempNum ,strTemp1;
			strTempNum.Format("%012s",pData[0]);
			int nTemp = strTempNum.GetLength();
			if(nTemp > 12) //号码长度不能大于12
			{
				return TRUE;
			}
			int nPoint = 0;
			int nNumCount = nTemp /2;
			nPoint = 0;
			for(int i = 0; i < nNumCount; i++)
			{
				strTemp1 = strTempNum.Mid(nPoint,2);
				nPoint = nPoint + 2;
				temp_GpsData.GpsId[i] = atoi(strTemp1);
			}
			temp_GpsData.dir = atoi(pData[9]);
			temp_GpsData.height = 0;
			temp_GpsData.speed = atoi(pData[8]);

			COleDateTime tempDTime;
			nYY = nYY + 2000;
			tempDTime.SetDateTime(nYY,nM,nDD,nHH,nMM,nSS);
			temp_GpsData.time = tempDTime.m_dt; //时间
